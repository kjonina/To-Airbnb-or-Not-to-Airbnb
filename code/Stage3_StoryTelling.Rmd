...
### Research Question
1.	What is the impact of AirBnB on Irish Rental Crisis? 
...
**Comment:**
Map of Ireland represents the density of AirBnB listings, highlighting the areas that are in RTZs (in red). Athough Dublin City, Cork City and Galway City are all in RTZs, it is possible to see the huge amount of AirBnB listings available, which can be affecting the Irish Rental Crisis.  
...
fig <- fig %>% add_markers(marker = list(opacity = 0.4),colors = rent_colors,
                hovertemplate = ~paste("County:", county, "<br>", 
                        "Price (in €):", rent_potential_annual_income, "<br>",
                        "Room Type", room_type, "<br>",
                        "Mininum stay (in nights)", minimum_nights, "<br>",
                        "To Rent or To Buy:",airbnb_potential_annual_income))
fig <- fig %>% layout(
...
```{r message=FALSE, echo=FALSE}
rpz_df %>%
  count(airbnb_potential_annual_income) %>%
  plot_ly(labels = ~airbnb_potential_annual_income, values = ~n, type = 'pie',
          textinfo = 'label+percent',
          hoverinfo = 'label+value',
          hole = 0.5,
...
entire_rpz_more_earnings_rent %>%
  count(airbnb_potential_annual_income, county) %>%
  mutate(county = fct_reorder(county, n, .desc = FALSE)) %>%
  plot_ly(y = ~county, x = ~n, color = ~airbnb_potential_annual_income,
          
          hovertemplate = ~paste("County:", county, "<br>",
                        "Accomodation Type:", airbnb_potential_annual_income, "<br>", 
                        "Number of AirBnB listings:", n)) %>%
...
```{r echo=FALSE}
private %>%
  count(rent_pressure_zone, airbnb_potential_annual_income) %>%
  mutate(room_type = fct_reorder(airbnb_potential_annual_income, n, .desc = TRUE)) %>%
  plot_ly(x = ~room_type, y = ~n, color = ~rent_pressure_zone,
...
```{r echo=FALSE}
private %>%
  count(airbnb_potential_annual_income, rent_pressure_zone) %>%
  # mutate(rent_pressure_zone = fct_reorder(rent_pressure_zone, n, .desc = TRUE)) %>%
  plot_ly(x = ~rent_pressure_zone, y = ~n, color = ~airbnb_potential_annual_income,
