...
file_path <- here("data", "final_airbnb_df.csv")
...
#                                  "RTZ:", rent_pressure_zone , "<br>", 
#                                  "Number of AirBnB listings:", count_distinct)) %>%
#   add_bars(showlegend = TRUE, colors = pressure_colors) %>%
#   layout(xaxis = list(title = FALSE),
#          yaxis = list(title = "Number of AirBnB listings"),
#          title = list(text = "How long are the most AirBnB listings Available?",
#                       x = 0.5))
```
**Comment:**
...
fig <- plot_geo(df, lat = ~latitude, lon = ~longitude, color = ~available_nights)
...
fig <- plot_geo(df, lat = ~latitude, lon = ~longitude, color = ~per_day_rent_AirBnB)
...
fig <- fig %>% layout(
   title = list(text = '<b>Based on Earnings PER DAY, <span style="color:#F58518;">Long-Term Leasing</span> seems\n bring in less earnings in all counties',
                      x = 0.5), geo = g)
...
**Comment:**
When comparing daily rent rate and daily AirBnB, for most properties, AirBnB brings in more earnings.
...
fig <- plot_geo(df, lat = ~latitude, lon = ~longitude, color = ~per_year_rent_AirBnB)
fig <- fig %>% add_markers(marker = list(opacity = 0.4),colors = rent_colors,
                hovertemplate = ~paste("County:", county, "<br>", 
                        ...
                        "To Rent or To Buy:",per_year_rent_AirBnB))
fig <- fig %>% layout(
    title = list(text = '<b>Based on Earnings PER YEAR, <span style="color:#F58518;">Long-Term Leasing</span> seems\n to bring in more revenue in <span style="color:#F58518;">all counties</span> except Tipperary</b>',
                      x = 0.5), geo = g)
...
entire %>%
  ...
  plot_ly(labels = ~per_year_rent_AirBnB, values = ~n, type = 'pie',
          ...
          hole = 0.45) %>%
  layout(title = list(text = '<b>Based on Earnings PER YEAR,\n <span style="color:#F58518;">Long-Term Leasing </span> an Entire Rental Unit would bring more earnings for 2726 listings</b>',
                      x = 0.5),
         showlegend = FALSE)
...
entire %>%
  ...
  layout(xaxis = list(title = FALSE),
          yaxis = list(title = "Number of AirBnB listings"),
         title = list(text = '<b>Based on Earnings PER YEAR,\n number of listings that would be profitable from <span style="color:#F58518;">Long-Term Lease </span> Entire Units are the same, regardless on Non-RPZs/RPZs</b>',
                      x = 0.5))
...
entire_rent_profit %>%
  ...
  layout(yaxis = list(title = FALSE),
          xaxis = list(title = "Number of AirBnB listings"),
         title = list(text = '<b>Based on Earnings PER YEAR, over 600 Entire Units would bring more earnings to  by <span style="color:#F58518;">Long-Term Leasing</span> Entire Units in Dublin',
                      x = 0.5))
   
```
```{r warning=FALSE, echo=FALSE}
entire_month_category_county_table <- entire_rent_profit %>%
...
print(entire_month_category_county_table)
```
```{r warning=FALSE, echo=FALSE}
...
crosstable_function(entire_month_category_county_table)
