...
# specifying the path to the CSV file
file_path <- here(DIRECTORY, "rent_in_ireland_2024.csv")
...
```{r}
galway_df <- df %>%
  filter(county_city == 'Galway City' | county_city == 'Galway County') 


Waterford_df <- df %>%
  filter(council == 'Waterford City And County Council' ) 

limerick_df <- df %>%
  filter(county_city == 'Limerick County' ) 

Waterford_df%>%
  group_by(council, lea) %>%
  summarise(count = n())
# unique(df$county_city)
...
fig <- plot_geo(limerick_df, lat = ~latitude, lon = ~longitude, color = ~lea)
fig <- fig %>% add_markers(marker = list(opacity = 0.2), showlegend=FALSE, colors = reverse_viridis,
...
###################
# Define the file path using here
file_path <- here(DIRECTORY, "airbnb_large_df.csv")
# Save the dataframe as a CSV file
...
Lyons, R. (2021) *Irish Rental Report Q1 2024.* Available at: https://ww1.daft.ie/report/ronan-lyons-2024q1-daftrentalprice/infographic?d_rd=1 (Accessed: 17 July 2024).
...
```{r}
df %>% 
  group_by(rent_council_city, room_type) %>%
  summarise(rent_potential_annual_income_per_month.x,total_income, rent_potential_annual_income_per_year)
```
